...
// --------------------         2024-08-16
// 独り言モード：テーマに基づいた豆知識を話す
String monologue_theme = "宇宙";  // テーマ設定（変更可能）
String random_prompts[12] = {
  "について、面白い豆知識を一つ簡潔に教えて",
  "に関する驚くべき事実を一つ教えて",
  "の不思議な現象について教えて",
  "に関する最近の発見を教えて",
  "について、あまり知られていない事実を教えて",
  "の興味深い話を一つ教えて",
  "に関する意外な真実を教えて",
  "について、子供でもわかるように説明して",
  "の魅力的な特徴を一つ教えて",
...
void loop()
{
  ...
  if (random_time >= 0 && millis() - lastms1 > random_time)
  {
    ...
    random_time = 40000 + 1000 * random(30);
    if (!mp3->isRunning() && speech_text=="" && speech_text_buffer == "") {
      exec_chatGPT(monologue_theme + random_prompts[random(12)]);
      mode = 0;
    }
  }
