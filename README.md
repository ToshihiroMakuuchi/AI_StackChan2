...
---
## 2026年1月2日: 独り言モード機能拡張
独り言モードを**テーマベースの豆知識**を話す機能に変更しました。<br>
ユーザーが設定したテーマ（初期設定：宇宙）に基づいて、AIｽﾀｯｸﾁｬﾝが自動的に豆知識を話します。<br>
テーマは`main.cpp`の`monologue_theme`変数を変更することで簡単にカスタマイズできます。（例：日本史、動物、科学など）<br>
40～69秒ごとに12種類のバリエーション豊かなプロンプトからランダムに選択し、ChatGPTに問い合わせます。<br><br>
...
* 画面左端の中央付近にタッチすると【独り言モード】に切り替えられます。<br>
  - 独り言モードをONにすると、40～69秒間隔でテーマに基づいた豆知識をAIｽﾀｯｸﾁｬﾝが自動的に話します。<br>
  - テーマ変更方法：`M5Unified_AI_StackChan/src/main.cpp`の1494行目付近の`monologue_theme`変数を編集<br>
  - 例：`String monologue_theme = "日本史";` → 日本史の豆知識を話すようになります<br>
